...
import {
  RADAR_EMAIL_STATUS_VERSION,
  RADAR_MARKETING_CONSENT_TEXT,
  RADAR_MARKETING_CONSENT_VERSION,
  RADAR_SERVICE_CONSENT_TEXT,
  ...
  normalizeMarketingPreference,
} from './radar-email.mjs';
...
function emailStatusBody(row, confirmationAvailable = true) {
  if (!row) return {
    version: RADAR_EMAIL_STATUS_VERSION, state: 'none', confirmation_available: confirmationAvailable,
  };
  return {
    version: RADAR_EMAIL_STATUS_VERSION,
    state: row.state,
    ...
    ...(row.state === 'pending' ? { confirmation_expires_at: row.confirmation_expires_at } : {}),
  };
}
...
export function createRadarSubscriptionsRouter({ database, environment = process.env, mailSender } = {}) {
  ...
  router.get('/email', async (request, response, next) => {
    try {
      if (!database) throw new RadarError(503, 'database_unavailable');
      const subscription = await transaction(database, async (client) => {
        const profile = await authenticatedProfile(request, client, environment);
        const result = await client.query(`
          SELECT state,email,marketing_consent,confirmation_expires_at
          FROM radar_email_subscriptions WHERE profile_id=$1
        `, [profile.id]);
        return result.rows[0] || null;
      });
      response.json(emailStatusBody(subscription, typeof mailSender === 'function'));
    } catch (error) { next(error); }
  });
  ...
  router.post('/email', async (request, response, next) => {
    try {
      if (!database) throw new RadarError(503, 'database_unavailable');
      if (typeof mailSender !== 'function') throw new RadarError(503, 'email_service_unavailable');
      requireSameOrigin(request, environment);
      requireJson(request);
      let normalized;
      let fingerprint;
      try {
        normalized = normalizeEmailRequest(request.body);
        fingerprint = emailFingerprint(normalized.email, environment);
      } catch (error) {
        if (error instanceof RadarEmailError) {
          throw new RadarError(error.code === 'email_service_unavailable' ? 503 : 400, error.code);
        }
        throw error;
      }
      const rawToken = randomBytes(32).toString('base64url');
      ...
      const confirmationsPerHour = configuredPositiveInteger(
        environment, 'RADAR_EMAIL_CONFIRMATIONS_PER_HOUR', 2_000,
      );
      const pending = await transaction(database, async (client) => {
        const profile = await authenticatedProfile(request, client, environment, { csrf: true, lock: true });
        await chargeProfileRate(client, profile.id, 'mutation', PROFILE_RATE_LIMITS.mutation);
        await chargeGlobalRate(client, 'email_confirmation', confirmationsPerHour);
        const owner = await client.query(`
          SELECT profile_id FROM radar_email_subscriptions
          WHERE email_fingerprint=$1 FOR UPDATE
        `, [fingerprint]);
        if (owner.rowCount && owner.rows[0].profile_id !== profile.id) return null;
        const current = await client.query(`
          SELECT state,email_fingerprint,resend_available_at
          FROM radar_email_subscriptions WHERE profile_id=$1 FOR UPDATE
        `, [profile.id]);
        if (current.rowCount && safeEqual(current.rows[0].email_fingerprint, fingerprint)
            && current.rows[0].state === 'active') {
          await client.query(`
            UPDATE radar_email_subscriptions SET
              service_consent_version=$2,service_consent_text=$3,service_consented_at=clock_timestamp(),
              marketing_consent=$4,marketing_consent_version=$5,marketing_consent_text=$6,
              marketing_consented_at=CASE WHEN $4 THEN clock_timestamp() ELSE NULL END,
              updated_at=clock_timestamp()
            WHERE profile_id=$1
          `, [
            profile.id, RADAR_SERVICE_CONSENT_VERSION, RADAR_SERVICE_CONSENT_TEXT,
            normalized.marketingConsent, RADAR_MARKETING_CONSENT_VERSION, RADAR_MARKETING_CONSENT_TEXT,
          ]);
          return { active: true };
        }
        if (current.rowCount && safeEqual(current.rows[0].email_fingerprint, fingerprint)
            && current.rows[0].resend_available_at > new Date()) return null;
        await client.query('DELETE FROM radar_email_suppressions WHERE email_fingerprint=$1', [fingerprint]);
        await client.query(`
          INSERT INTO radar_email_subscriptions(
            ...
            marketing_consent,marketing_consent_version,marketing_consent_text,marketing_consented_at
          ) VALUES(
            $1,$2,$3,'pending',$4,clock_timestamp()+interval '24 hours',
            clock_timestamp()+interval '7 days',clock_timestamp()+make_interval(mins=>$5),
            $6,$7,clock_timestamp(),$8,$9,$10,CASE WHEN $8 THEN clock_timestamp() ELSE NULL END
          ) ON CONFLICT(profile_id) DO UPDATE SET
            email=excluded.email,email_fingerprint=excluded.email_fingerprint,state='pending',
            confirmation_token_hash=excluded.confirmation_token_hash,
            confirmation_expires_at=excluded.confirmation_expires_at,
            unconfirmed_delete_at=excluded.unconfirmed_delete_at,
            resend_available_at=excluded.resend_available_at,
            service_consent_version=excluded.service_consent_version,
            service_consent_text=excluded.service_consent_text,
            service_consented_at=excluded.service_consented_at,
            ...
            requested_at=clock_timestamp(),confirmed_at=NULL,updated_at=clock_timestamp()
          RETURNING profile_id,email,confirmation_token_hash,confirmation_expires_at
        `, [
          profile.id, normalized.email, fingerprint, tokenHash, EMAIL_RESEND_MINUTES,
          RADAR_SERVICE_CONSENT_VERSION, RADAR_SERVICE_CONSENT_TEXT,
          normalized.marketingConsent, RADAR_MARKETING_CONSENT_VERSION, RADAR_MARKETING_CONSENT_TEXT,
        ]);
        return { profileId: profile.id, email: normalized.email, tokenHash };
      });

      if (pending?.active) return response.status(200).json({ version: RADAR_EMAIL_STATUS_VERSION, state: 'active' });
      if (pending) {
        const customId = `confirm:${pending.profileId}:${dispatchId}`;
        let receipt = null;
        try {
          receipt = await mailSender(confirmationMessage({
            email: pending.email, token: rawToken, customId,
          }, environment));
        } catch {
          await database.query(`
            UPDATE radar_email_subscriptions SET resend_available_at=clock_timestamp()
            WHERE profile_id=$1 AND state='pending' AND confirmation_token_hash=$2
          `, [pending.profileId, pending.tokenHash]).catch(() => {});
        }
        if (receipt) {
          await transaction(database, async (client) => {
            const current = await client.query(`
              SELECT 1 FROM radar_email_subscriptions
              WHERE profile_id=$1 AND state='pending' AND confirmation_token_hash=$2
            `, [pending.profileId, pending.tokenHash]);
            if (!current.rowCount) return;
            await client.query(`
              INSERT INTO radar_email_deliveries(
                id,profile_id,custom_id,kind,provider_message_id,provider_message_uuid
              ) VALUES($1,$2,$3,'confirmation',$4,$5)
            `, [randomUUID(), pending.profileId, customId, receipt.messageId, receipt.messageUuid]);
          }).catch(() => {});
        }
      }
      response.status(202).json({ version: RADAR_EMAIL_STATUS_VERSION, state: 'pending' });
    } catch (error) {
      if (error?.code === '23505'
          && error?.constraint === 'radar_email_subscriptions_email_fingerprint_key') {
        return response.status(202).json({ version: RADAR_EMAIL_STATUS_VERSION, state: 'pending' });
      }
      next(error);
    }
  });

  router.post('/email/confirm', async (request, response, next) => {
    try {
      if (!database) throw new RadarError(503, 'database_unavailable');
      requireSameOrigin(request, environment);
      requireJson(request);
      let token;
      try { token = normalizeConfirmation(request.body); }
      catch (error) {
        if (error instanceof RadarEmailError) throw new RadarError(400, 'confirmation_invalid');
        throw error;
      }
      const confirmed = await transaction(database, async (client) => client.query(`
        UPDATE radar_email_subscriptions subscription SET
          state='active',confirmation_token_hash=NULL,confirmation_expires_at=NULL,
          unconfirmed_delete_at=NULL,resend_available_at=NULL,
          confirmed_at=clock_timestamp(),updated_at=clock_timestamp()
        FROM radar_profiles profile
        WHERE subscription.profile_id=profile.id
          ...
          AND subscription.state='pending' AND subscription.confirmation_token_hash=$1
          AND subscription.confirmation_expires_at>clock_timestamp()
        RETURNING subscription.profile_id
      `, [sha256(token)]));
      if (!confirmed.rowCount) throw new RadarError(400, 'confirmation_invalid');
      response.json({ version: RADAR_EMAIL_STATUS_VERSION, state: 'active' });
    } catch (error) { next(error); }
  });

  router.put('/email/marketing', async (request, response, next) => {
    try {
      if (!database) throw new RadarError(503, 'database_unavailable');
      requireSameOrigin(request, environment);
      requireJson(request);
      let marketingConsent;
      try { marketingConsent = normalizeMarketingPreference(request.body); }
      catch (error) {
        if (error instanceof RadarEmailError) throw new RadarError(400, 'invalid_email_request');
        throw error;
      }
      const updated = await transaction(database, async (client) => {
        ...
        return client.query(`
          UPDATE radar_email_subscriptions SET
            marketing_consent=$2,marketing_consent_version=$3,marketing_consent_text=$4,
            marketing_consented_at=CASE WHEN $2 THEN clock_timestamp() ELSE NULL END,
            updated_at=clock_timestamp()
          WHERE profile_id=$1 AND state='active'
          RETURNING state,email,marketing_consent,confirmation_expires_at
        `, [profile.id, marketingConsent, RADAR_MARKETING_CONSENT_VERSION, RADAR_MARKETING_CONSENT_TEXT]);
      });
      if (!updated.rowCount) throw new RadarError(404, 'email_subscription_unavailable');
      response.json(emailStatusBody(updated.rows[0], typeof mailSender === 'function'));
    } catch (error) { next(error); }
  });
  ...
  router.delete('/email', async (request, response, next) => {
    try {
      ...
      requireSameOrigin(request, environment);
      requireJson(request);
      if (!exactObject(request.body, [])) throw new RadarError(400, 'invalid_email_request');
      await transaction(database, async (client) => {
        const profile = await authenticatedProfile(request, client, environment, { csrf: true, lock: true });
        await chargeProfileRate(client, profile.id, 'mutation', PROFILE_RATE_LIMITS.mutation);
        await client.query('DELETE FROM radar_email_subscriptions WHERE profile_id=$1', [profile.id]);
      });
      response.status(204).end();
    } catch (error) { next(error); }
  });
