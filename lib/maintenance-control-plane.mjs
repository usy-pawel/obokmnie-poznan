...
const ISSUE_POLICY = new Map([
  ['daily_import_failed', {
    capability: 'daily_import', severity: 'P0', owner: 'data_pipeline', next_action_code: 'inspect_latest_import',
  }],
  ['daily_import_stale', {
    capability: 'daily_import', severity: 'P1', owner: 'data_pipeline', next_action_code: 'run_import_preflight',
  }],
  ['invalid_data_coverage', {
    capability: 'data_coverage', severity: 'P0', owner: 'data_pipeline', next_action_code: 'inspect_import_coverage',
  }],
  ['events_for_non_success_import', {
    capability: 'radar_diff', severity: 'P0', owner: 'radar_diff', next_action_code: 'inspect_event_integrity',
  }],
]);
  ...
  try {
    ...
    const result = observations.map((item) => {
      ...
      if (item.health === 'unhealthy') {
        const policy = ISSUE_POLICY.get(item.code);
...
function validateRemainingIssue(row) {
  const policy = ISSUE_POLICY.get(row?.code);
