...
export async function watchdog(database) {
  return transaction(database, async (client) => {
    ...
    if (!lease.run_id) {
      return {
        status: 'idle', code: null, observed_at: timestamp(lease.observed_at), effects_performed: false,
      };
    }
    ...
    if (!run || run.executor !== lease.owner || run.fence !== lease.fence
        || run.context_hash !== lease.context_hash) throw controlError('lease_state_invalid');
    const handle = {
      run_id: run.id,
      owner: run.executor,
      fence: run.fence,
    ...
    if (run.status !== 'running') {
      await clearLease(client, handle);
      return {
        status: 'idle', code: 'terminal_lease_cleared',
        observed_at: timestamp(lease.observed_at), effects_performed: false,
      };
    }
    const unexpired = isAfter(lease.expires_at, lease.observed_at)
      && isAfter(run.deadline_at, lease.observed_at);
    if (unexpired) {
      return {
    ...
    return {
      ...await materializeTimeout(client, handle),
      observed_at: timestamp(lease.observed_at),
...
export async function acquire(database, invocationKey, contextHash, executor) {
  ...
  return transaction(database, async (client) => {
    ...
    if (existing) {
      if (existing.status === 'running' && existing.executor !== safeExecutor) {
        return { status: 'busy', code: 'lease_busy' };
      }
...
export async function completeHealthSweep(database, rawHandle, observations) {
  ...
  return transaction(database, async (client) => {
    const active = await lockHandle(client, handle);
    if (active.expired) return materializeTimeout(client, handle);
    if (active.actions_disabled !== true) {
      return failActiveRun(client, handle, 'control_plane_failed');
    }
...
async function failActiveRun(client, handle, code) {
  const remaining = await remainingIssues(client);
...
export async function failRun(database, rawHandle, typedCode) {
  ...
  return transaction(database, async (client) => {
    const active = await lockHandle(client, handle);
    if (active.expired) return materializeTimeout(client, handle);
    return failActiveRun(client, handle, code);
  });
}
