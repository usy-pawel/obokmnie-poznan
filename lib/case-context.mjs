...
export function buildContextFacts(subject, relatedCases) {
  ...
  return {
    ...
    parcel_history: {
      other_cases_on_same_parcel: Number(sameParcelCases[0]?.same_parcel_count || sameParcelCases.length || 0),
...
export function deterministicContext(facts) {
  ...
  const parcelHistory = facts.parcel_history;
  ...
  if (parcelHistory.other_cases_on_same_parcel) {
    signals.push(`Na tych samych działkach znaleziono ${parcelHistory.other_cases_on_same_parcel} innych spraw w bieżącym zakresie danych.`);
  }
  return {
