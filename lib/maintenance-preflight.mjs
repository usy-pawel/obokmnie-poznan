...
export const PREFLIGHT_VERSION = 'radar_maintenance_paper_v1';
...
function selectedIssue(databaseReachable, dataStatus) {
  ...
  if (dataStatus?.status === 'failed') {
    const policy = maintenanceIssuePolicy('daily_import_failed');
    return { severity: policy.severity, code: 'daily_import_failed', owner: policy.owner };
  }
  if (dataStatus?.status === 'stale') {
    const policy = maintenanceIssuePolicy('daily_import_stale');
    return { severity: policy.severity, code: 'daily_import_stale', owner: policy.owner };
  }
